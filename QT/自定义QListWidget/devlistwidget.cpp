...
DevListWidget::DevListWidget(QListWidget *parent) :
    QListWidget(parent)
{
...
void DevListWidget::initMenu()
{
   ...
   //connect(groupNameEdit,SIGNAL(editingFinished()),this,SLOT(slotRenameEditFshed()));
    ...
    QListWidgetItem *newItem=new QListWidgetItem(QIcon("C:/Users/Administrator/Desktop/share/TP/right"),"设备列表");    //创建一个Item
...
//鼠标点击事件
void DevListWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if(event->button()==Qt::LeftButton && currentItem!=NULL && currentItem==groupMap.value(currentItem))//如果点击的左键并且是点击的是组
    {
        if(isHideMap.value(currentItem))                                  //如果先前是隐藏，则显示
        {
            ...
            currentItem->setIcon(QIcon("C:/Users/Administrator/Desktop/share/TP/bottom"));
        }
        else                                                             //否则，先前是显示，则隐藏
        {
            foreach(QListWidgetItem* subItem, groupMap.keys(currentItem))//遍历组的对应的项（包括自身和好友）
             ...
             currentItem->setIcon(QIcon("C:/Users/Administrator/Desktop/share/TP/right"));
        }
    }
}
//菜单事件，为了显示菜单，点击鼠标右键响应，鼠标点击事件mousePressEvent优先于contextMenuEvent
void DevListWidget::contextMenuEvent(QContextMenuEvent *event)
{
...
//添加组
void DevListWidget::slotAddGroup()
{
    QListWidgetItem *newItem=new QListWidgetItem(QIcon("C:/Users/Administrator/Desktop/share/TP/right"),"设备列表");    //创建一个Item
    newItem->setSizeHint(QSize(this->width(),25));//设置宽度、高度
    this->addItem(newItem);         //加到QListWidget中
...
//删除组
void DevListWidget::slotDelGroup()
{
...
void DevListWidget::slotAddBuddy()
{
    personListBuddy *buddy=new personListBuddy();   //创建一个自己定义的信息类
    buddy->headPath="C:/Users/Administrator/Desktop/share/TP/timg1.jpg";                          //设置头像路径
...
//删除好友
void DevListWidget::slotDelBuddy()
{
    groupMap.remove(currentItem);  //移除该好友
    delete currentItem;            //删除
}
//重命名完成
void DevListWidget::slotRenameEditFshed()
{
